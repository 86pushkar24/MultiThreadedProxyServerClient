#include "proxy_parse.h" // Header file for parsing HTTP requests
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/types.h>
...
void *thread_fn(void *socketNew)
{
	...
	int bytes_send_client, len; // Bytes Transferred from clients 
	...
	{
		len = strlen(buffer);
		// loop until u find "\r\n\r\n" in the buffer as this is the end of the request
		if (strstr(buffer, "\r\n\r\n") == NULL) 
		{
	...
	if (temp != NULL)
	{
		...
		while (pos < size)
		{
			...
			{
				response[i] = temp->data[pos]; // Copying the data from cache element to response
				pos++;
			}
...
int main(int argc, char *argv[])
{
	...
	proxy_socketId = socket(AF_INET, SOCK_STREAM, 0); // AF_INET: IPv4, SOCK_STREAM: TCP
	...
	// If port is already in use, we set the SO_REUSEADDR option to allow reuse of the port
	int reuse = 1;
	...
	while (1)
	{
		...
		struct sockaddr_in *client_pt = (struct sockaddr_in *)&client_addr; // Typecasting client_addr to sockaddr_in pointer
		struct in_addr ip_addr = client_pt->sin_addr; // Extracting ip address from client_addr
